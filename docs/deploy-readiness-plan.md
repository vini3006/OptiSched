...
- **API + Optimizer (EC2) — a implementar:**
  ...
  - [x] **Validado em produção (2026-08-07)**: pipeline completo rodou de ponta a ponta — push na `main` → testes (API/Optimizer/Web) → build+push das imagens no GHCR → SSH na EC2 → `docker compose pull && up -d` → smoke test → confirmado também de forma independente (`curl -I https://api.optisched.com.br/actuator/health` → `200`).
    - **Achado 1 — CVEs críticas bloqueando o gate do A7**: a primeira rodada real expôs `netty-transport-4.1.135.Final` (CVE-2026-56820, CVSS 9.1) e `tomcat-embed-core-10.1.55` (4 CVEs, CVSS 9.1), ambas geridas pelo `spring-boot-starter-parent:3.5.16`. Corrigido sobrescrevendo `netty.version`/`tomcat.version` no `pom.xml` da API pras versões patch mais novas da mesma linha (4.1.137.Final / 10.1.57), sem subir o Spring Boot inteiro.
    - **Achado 2 — cache do NVD nunca salva em job que falha**: comportamento documentado do `actions/cache` (só salva se o job inteiro passar), não bug de config — resolvido pelo Achado 1 (job passando, cache salvando, runs seguintes bem mais rápidas).
    - **Achado 3 — SSH bloqueado pelo Security Group**: a EC2 só aceita SSH do IP do desenvolvedor (decisão da Parte B); o runner do GitHub usa IP dinâmico. Resolvido com um par de steps novos no job `deploy` que autorizam a porta 22 pro IP do runner via AWS CLI antes do SSH e revogam depois (`if: always()`) — porta nunca fica aberta pra `0.0.0.0/0` permanentemente. Precisou de usuário IAM novo (`github-actions-deploy`, policy inline restrita só a essa Security Group) e 3 secrets novos (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `EC2_SECURITY_GROUP_ID`).
    - **Achado 4 — região AWS errada**: `ci.yml` tinha `aws-region: us-east-1` (registro antigo, de quando a conta só liberava as regiões "starter"); a EC2 na verdade está em `sa-east-1`. Corrigido no workflow e na policy IAM.

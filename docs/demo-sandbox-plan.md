...
## Fase 2 — Backend: `DemoSeedService` (dados de exemplo)

- [ ] `service/DemoSeedService.java` (novo): `seedUniversity(Long institutionId)` e `seedSchool(Long institutionId)`, cada um chamando os `create(...)` já existentes de cada serviço de domínio (não CSV) — `CourseService`, `SubjectService`, `UserService.createProfessor`, `ClassroomService`, `TimeSlotService`, `SemesterService` no lado universidade; `SerieService`, `TurmaService`, `SerieSubjectService` no lado escola; `ProfessorQualificationService`, `AvailabilityService` em ambos. Sem `@Transactional` de método (mesmo espírito dos importadores CSV — uma falha isolada não deve derrubar a instituição inteira, que já tem um ADMIN válido). Ordem respeitando FKs: semestre/ano letivo e cursos/séries antes de disciplinas; disciplinas antes de qualificações; professores antes de disponibilidade.
- [ ] Dataset pequeno de propósito (roda síncrono dentro do request público rate-limitado): ~2 cursos/séries, ~3-4 turmas/ofertas, ~6 disciplinas, ~4 professores com qualificações+disponibilidade cobrindo a maioria dos horários, 2-3 salas, ~20 horários (uma semana de manhã), 1 semestre/ano letivo.
- [ ] Estender `DemoIntegrationTest.java` (não criar teste novo) pra assertar contagens de linhas semeadas (`courseRepository.countByInstitutionId(id) > 0` etc.).
